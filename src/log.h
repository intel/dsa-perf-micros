...
static inline void __attribute__((always_inline, format(printf, 2, 3)))
	log_null(struct log_ctx *ctx __attribute__((unused)),
...
#define ERR(...) log_err(&log_ctx, __VA_ARGS__)
#define INFO(...) log_info(&log_ctx, __VA_ARGS__)

#define INFO_CPU(tcpu, ...) \
do {\
	log_info(&log_ctx, "CPU %d: ", tcpu->cpu_num);\
	log_info(&log_ctx, __VA_ARGS__);\
} while (0)

#define DEBUG_CPU(tcpu, ...) \
do {\
	log_dbg(&log_ctx, "CPU %d: ", tcpu->cpu_num);\
	log_dbg(&log_ctx, __VA_ARGS__);\
} while (0)
