...
static uint64_t
iter_count(struct tcfg *tcfg)
{
	static uint64_t *curr_iter;
	uint64_t iter_diff = 0;
	unsigned int i;

	if (!curr_iter)
		curr_iter = calloc(1, tcfg->nb_cpus * sizeof(*curr_iter));
	...
	for (i = 0; i < tcfg->nb_cpus; i++) {
		...
		iter = tcfg->tcpu[i].curr_iter;
		iter_diff += iter - curr_iter[i];
		curr_iter[i] = iter;
	}
